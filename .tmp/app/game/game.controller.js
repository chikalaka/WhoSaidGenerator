...
angular.module('whoSaidApp').controller('GameController', function ($scope, $http, Auth) {
    ...
    $scope.postSentence = function () {
        ...
        */
        ...
        $http.get('/api/groups/' + $scope.groupSelectedId).success(function (data) {
            var group = data;
            //group.phrases.push($scope.newPhrase);
            console.log(group);

            $http.put('/api/groups/' + group._id, $scope.newPhrase).success(function () {
